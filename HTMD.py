...
system(
    f"{gmx} pdb2gmx -ff charmm36m -f {receptor} -o Receptor_gmx.pdb -water tip3p -ignh -p topol.top {null}"
)
...
for mol2 in ligList:
    ...
    system(
        f"{gmx} pdb2gmx --ff {filename} -f {filename}.pdb -o Ligand_gmx.pdb -p Ligand.top {null}"
    )
    ...
    system(
        f"{gmx} grompp -f {ions_mdp} -c {Complex.name} -p topol.top -o Complex_b4ion.tpr -maxwarn 10 {null}"
    )
    system(
        f"echo 15 | {gmx} genion -s Complex_b4ion.tpr -o Complex_4mini.pdb -neutral -conc 0.15 -p topol.top {null}"
    )

    system(
        f"""{gmx} make_ndx -f Complex_4mini.pdb -o index.ndx << EOF
1 | 20
q
EOF"""
    )
    ...
    system(
        f"{gmx} grompp -f {mini_mdp} -c Complex_4mini.pdb -r Complex_4mini.pdb -p topol.top -o mini.tpr -maxwarn 10 {null}"
    )
    system(f"{mdrun} -deffnm mini -nt {nt} -gpu_id {gpu_id} {null}")

    equi_mdp = make_mdp(mdp="equi")
    system(
        f"{gmx} grompp -f {equi_mdp} -c mini.gro -r mini.gro -p topol.top -o equi.tpr -maxwarn 10 {null}"
    )
    system(f"{mdrun} -deffnm equi -nt {nt} -gpu_id {gpu_id}")

    MD_mdp = make_mdp(mdp="MD")
    system(
        f"{gmx} grompp -f {MD_mdp} -c equi.gro -p topol.top -o MD.tpr -maxwarn 10 {null}"
    )
    system(f"{mdrun} -v -deffnm MD -nt {nt} -gpu_id {gpu_id}")

    system(f"{gmx} editconf -f MD.tpr -o MD.pdb {null}")
    system(
        f"""{gmx} mindist -f MD.xtc -s MD.tpr -d 0.45 -n index.ndx -on contacts.xvg << EOF
    Protein
    13
    EOF"""
    )
    system(
        f"""{gmx} rms -f MD.xtc -s MD.tpr -n index.ndx << EOF
    24
    13
    EOF"""
    )
