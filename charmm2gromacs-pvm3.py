...
funcForDihedrals = (
    "9"  # special type for treating multiple entries (modification in source code)
)
funcForImpropers = "2"
    ...
    if len(line) > 1:
        ...
        if mass and not postMass:
            ...
            if line[0] != comment:
                ...
                atpFile.write(type + "\t" + mass + " " + ";" + "\t" + string + "\n")
                # set dictionaries:
    ...
    if len(line) > 0:
        ...
        if angles and not dihedrals:
            ...
            if line[0] != comment and segments[0] != "ANGLES":
                ...
                cth = str(
                    float(cth) * 2 * kcal2kJ
                )  # -> kJ/mol and an factor 2 (see definitions)
        ...
        if impropers and not nonbonded and not cmap:
            ...
            if line[0] != comment and not segments[0].startswith("IMPROPER"):
                ...
                if typej == "X" and typek == "X":
                    ...
                    cq = str(
                        float(cq) * 2 * kcal2kJ
                    )  # factor 2 from definition difference
                    bonFile.write(
                        typei
                        + "\t"
                        + typej
        ...
        if nonbonded and not hbond:
            ...
            if (
                line[0] != comment
                and segments[0] != "NONBONDED"
                and segments[0] != "cutnb"
                and segments[0] != "CUTNB"
            ):
                type = segments[0]
                epsilon = segments[2]
                ...
                RminHalf = segments[3]
                sigma = str(
                    2 * float(RminHalf) / (10.0 * 2.0 ** (1.0 / 6.0))
                ...
                if len(segments) > 6:
                    ...
                    if not isinstance(segments[5], str):  # not string?
                        ...
                        # conversion to gromacs units
                        sigma14 = str(
                            2 * float(Rmin14Half) / (10.0 * 2.0 ** (1.0 / 6.0))
                        )
