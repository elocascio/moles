...
parser.add_argument(
    ...
    "--nanoseconds",
    type=int,
    help="ns of simulation --- default 5 ns",
    default=5,
)
parser.add_argument(
    "-step", 
...
parser.add_argument(
    ...
    "--lignum",
    type=int,
    help="send mail every n ligands --- default 50",
    default=50,
)
parser.add_argument(
    "-p", "--pool", 
...
parser.add_argument(
    ...
    "--ligname",
    type=str,
    help="ligand common name or mol2 ligand file w/o ext --- default ZINC",
    default="ZINC",
)
parser.add_argument(
    "-i", "--ligID", 
...
parser.add_argument(
    ...
    "--receptor",
    type=str,
    help="receptor pdb --- default protein.pdb",
    default="protein.pdb",
)
parser.add_argument(
    "-f",
    "--report",
    type=str,
    help="path of report --- default ./report.csv",
    default="./report.csv",
)
...
system(
    f"{args.gmx} pdb2gmx -ff charmm36m -f {args.receptor} -o Receptor_gmx.pdb -water tip3p -ignh -p topol.top {null}"
)
...
def main(mol2):
    ...
    system(
        f"python {charmm2gmx} {getcwd()}/{filename}.rtf {getcwd()}/{filename}.prm {filename}.ff"
    )
    ligand_ff = f"{filename}.ff"
    system(f"obabel -imol2 {mol2} -opdb -O {filename}.pdb {null}")
    system(
        f"{args.gmx} pdb2gmx --ff {filename} -f {filename}.pdb -o Ligand_gmx.pdb -p Ligand.top {null}"
    )
...
#include "posre.itp"
    ...
    with open("../Receptor_gmx.pdb") as receptor_gmx, open(
        f"{filename}.pdb"
    ) as ligand_pdb, open("Complex.pdb", "w") as Complex:
    ...
    system(
        f"{args.gmx} solvate -cp {Complex.name} -o {Complex.name} -p topol.top -quiet"
    )

    ions_mdp = make_mdp("ions")
    system(
        f"{args.gmx} grompp -f {ions_mdp} -c {Complex.name} -p topol.top -o Complex_b4ion.tpr -maxwarn 10 -quiet {null}"
    )
    system(
        f"echo 15 | {args.gmx} -quiet genion -s Complex_b4ion.tpr -o Complex_4mini.pdb -neutral -conc 0.15 -p topol.top -quiet {null}"
    )

    system(
        f"""{args.gmx} -quiet make_ndx -f Complex_4mini.pdb -o index.ndx << EOF
1 | 20
q
EOF"""
    )
    ...
    system(
        f"{args.gmx} grompp -f {mini_mdp} -c Complex_4mini.pdb -r Complex_4mini.pdb -p topol.top -o mini.tpr -maxwarn 10"
    )
    ...
    system(
        f"{args.gmx} grompp -f {equi_mdp} -c mini.gro -r mini.gro -p topol.top -o equi.tpr -maxwarn 10 {null}"
    )
    deviceID = gpu_manager()
    ...
    system(
        f"{args.gmx} grompp -f {MD_mdp} -c equi.gro -p topol.top -o MD.tpr -maxwarn 10 {null}"
    )
    deviceID = gpu_manager()
    ...
    system(
        f"""{args.gmx} mindist -f MD_pbc.xtc -s MD.tpr -d 0.45 -n index.ndx -on contacts.xvg << EOF
    Protein
    13
    EOF"""
    )
    system(
        f"""{args.gmx} rms -f MD_pbc.xtc -s MD.tpr -n index.ndx << EOF
    24
    13
    EOF"""
    )
    time, all_contacts, all_contacts_fig = plot_xvg(
        "contacts.xvg", "Number of Contacts", "contacts.png", "Time", "Concats"
    )
    time, rmsd, _ = plot_xvg("rmsd.xvg", "RMSD", "rmsd.png", "Time", "RMSD (A)")
    status, contacts_mean = detachmet(all_contacts)
    fig_contacts = contacts(
        xtc="MD_pbc.xtc", step=args.nanoseconds / 50, ligand=args.ligID
    )
    result = [
        filename,
        status,
