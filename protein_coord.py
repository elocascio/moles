...
def protein_contacts(topol = 'MD.pdb', trj = 'MD.xtc', step = 10, ligand = "segid PROB"):
    ...
    Polar = pd.DataFrame(); Polar_time = pd.DataFrame()
    HBond = []
    coord_dfs = []
...
############## Salt bridge ##############
        ...
        for position in lig_neg.positions:
            scores = switch(np.linalg.norm(prot_pos.positions - position, axis = 1), r_0 = 5)
            df_Sn[next(gen)] = scores
        df_Sn = pd.DataFrame((prot_pos_list, df_Sn.sum(axis = 1))).T
        df_Sn[1] = df_Sn[1].astype(float); df_Sn = df_Sn.groupby(0).mean()
        # neg vs pos #
        df_Sp = pd.DataFrame()
        ...
        for position in lig_pos.positions:
            scores = switch(np.linalg.norm(prot_neg.positions - position, axis = 1), r_0 = 5)
            df_Sp[next(gen)] = pd.Series(scores)
        df_Sp = pd.DataFrame((prot_neg_list, df_Sp.sum(axis = 1))).T
        df_Sp[1] = df_Sp[1].astype(float); df_Sp = df_Sp.groupby(0).mean()
        df_S = pd.DataFrame(); df_S = pd.concat([df_Sn, df_Sp])
        SaltBridge = pd.concat([SaltBridge, df_S]); SaltBridge_time = pd.concat([SaltBridge_time, df_S], axis = 1)
    ############## Salt bridge ##############
    ############## Polar ###############
        pos = []
        df_pol = pd.DataFrame()
        lig_back_polar = u.select_atoms(f"{ligand} and (backbone) and (name O* N*)")
        lig_polar = u.select_atoms(f"{ligand} and (resname THR SER GLN ASN CYS MET HSD TRP ) and (name O* N* S*)")
...
############# H-bond ##################### 
    ...
    for val in resulta:
        HBond.append([str(u.atoms[val[1]].resnum) + str(u.atoms[val[1]].resname), val[3]])
    for val in resultb:
        HBond.append([str(u.atoms[val[2]].resnum) + str(u.atoms[val[2]].resname), val[3]])
    h = pd.DataFrame(HBond); h.reset_index()
...
############ FINAL ################
    ...
    ax = df_all.plot.bar(stacked = True, color = ["purple", "mediumblue","lightgreen","aqua"])
    ax.set_ylim(top=4)
