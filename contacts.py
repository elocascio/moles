...
#------------------------------- Hydrophobic ----------------------------------
    ...
    for at in prot_c:
        ...
        coord_C = contacts.Contacts(u, select = (c_lig, prot), refgroup = (lig_c, sel), method=switch_function, kwargs={'r_0':6, 'a':6, 'b':12}).run(step=100)
        H.append([str(at.residue.resnum) + str(at.residue.resname) , np.mean(coord_C.timeseries[:, 1])])
    print('H fatto')    
#------------------------------- Polar -------------------------------------------
    P = []
    p_lig = "(resname UNK) and (type O* N* S*)"
    lig_p = u.select_atoms(p_lig)
    prot_p = u.select_atoms("(around 10 resname UNK) and (name N* O* S*) and not (resname SOL)")
    for at in prot_p:
        resid = at.resid
        name = at.name
        prot = f"(around 10 resname UNK) and (resid {resid}) and (name {name})"
