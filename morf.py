...
system(f'python {charmm2gmx} {getcwd()}/{args.lig}.rtf {getcwd()}/{args.lig}.prm {args.lig}.ff')
system(f'obabel -imol2 {args.lig}.mol2 -opdb -O {args.lig}.pdb')
system(f'{args.gmx} pdb2gmx --ff {args.lig} -f {args.lig}.pdb -o Ligand_gmx.pdb -p Ligand.top')
ligand_ff =f"{args.lig}.ff"
with open(f'{ligand_ff}/ffbonded.itp') as ffbonded, open(f'{ligand_ff}/ffnonbonded.itp') as ffnonbonded, open(f'Ligand.top') as ligand_top, open(f'{args.lig}.itp', 'w') as ligand_itp:
...
new_system = mda.Merge(mother_universe.select_atoms("all and not resname UNK"), (ligand_universe.atoms))
...
new_system.select_atoms("protein or resname UNK").write("index.ndx",
                                                        name = "SOLU",
                                                        mode="a")
new_system.select_atoms("resname TIP3 POT CLA").write("index.ndx",
                                             name = "SOLV",
                                             mode = "a")
new_system.select_atoms("resname POPC").write("index.ndx",
                                             name = "MEMB",
                                             mode = "a")
new_system.select_atoms("protein or (resname UNK) or (resname POPC)").write("index.ndx",
                                                                            name = "SOLU_MEMB",
                                                                            mode = "a")

system("echo 0 | gmx genrestr -f ../Ligand_gmx.pdb -o posre.itp -fc 1000 1000 1000")
system("gmx grompp -f step6.0_minimization.mdp -o step6.0_minimization.tpr -c morfed.gro -r morfed.gro -p morftopol.top -n index.ndx -maxwarn 10")
