...
def DNAze_pdb(pdb):
    ...
    with open(pdb, 'rt') as ref:
        data = ref.read()
        data = data.replace('CYT', ' DC').replace('ADE', ' DA').replace('GUA', ' DG').replace('THY', ' DT')
...
os.makedirs(torsion_folder, exist_ok = True); os.chdir(torsion_folder)
...
for chain in u.segments.segids[:-1]:
        ...
        for res, resnum in topol:
            try:
                ...
                plt.scatter(tsne[:ref.shape[0], 0], tsne[:ref.shape[0], 1], c = 'r', marker='+', alpha = 0.5)
                plt.scatter(tsne[ref.shape[0]:, 0], tsne[ref.shape[0]:, 1], c = 'blue', marker= '+', alpha = 0.5)
