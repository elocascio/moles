...
def protein_contacts(pdb="MD.pdb", xtc="MD.xtc", step=10):
        ...
        if ts.time % step == 0:
            ...
            for pi_stacking in my_interactions.pistacking:
                if pi_stacking.type == "T":
                    PiStacking_T.append(
                        [
                            str(pi_stacking.reschain)
                            + str(pi_stacking.resnr)
                            + str(pi_stacking.restype),
                            switch(float(pi_stacking.distance), r_0=5.5),
                        ]
                    )
            ...
            for salt_bridge in my_interactions.saltbridge_lneg:
                SaltBridge.append(
                    [
                        str(salt_bridge.reschain)
                        + str(salt_bridge.resnr)
                        + str(salt_bridge.restype),
                        switch(float(salt_bridge.distance), r_0=5),
                    ]
                )
            ...
            for h_bond in my_interactions.hbonds_ldon:
                HBond.append(
                    [
                        str(h_bond.reschain) + str(h_bond.resnr) + str(h_bond.restype),
                        str(h_bond.type),
                    ]
                )
    ...
    for coord, coord_type in list(
        ...
        if coord in [HBond, WaterBridge]:
            ...
            df[coord_type] = df[coord_type].apply(
                lambda x: x / (len(u.trajectory) / step)
            )
            coord_dfs.append(df)
        else:
            df = pd.DataFrame(coord, columns=["residue", coord_type])
    ...
    # POLAR CONTACTS
    pep_polar_str = "(segid D) and not (backbone) and (name O* N* S*) and not (resname ARG LYS ASP GLU)"
    pep_polar = u.select_atoms(pep_polar_str)
    pep_cation_str = "(segid D) and not (backbone) and (name N*) and (resname ARG LYS )"
    pep_cation = u.select_atoms(pep_cation_str)
    protein_polar_str = (
        "(around 10 segid D) and (name O* N* S*) and (resname THR SER ASN GLN CYS MET)"
    )
    protein_polar = u.select_atoms(protein_polar_str)
    protein_cation_str = (
        "(around 10 segid D) and not (backbone) and (name N*) and (resname ARG LYS)"
    )
    protein_cation = u.select_atoms(protein_cation_str)
    ...
    P.append(
        [
            str(at.chainID) + str(at.residue.resnum) + str(at.residue.resname),
            np.mean(coord_P.timeseries[:, 1]),
        ]
    )
