...
def media(coord, frames):
    return coord/frames

def contacts(pdb = 'MD.pdb', xtc = 'MD.xtc'):
    Hydrophobic = []
    PiStacking_T = []
    PiStacking_P = []
    PiCation = []
    ...
    for coord, coord_type in list(zip([Hydrophobic, PiStacking_T, PiStacking_P, SaltBridge, HBond, PiCation], ['Hydrophobic', 'Pi_stacking_T', 'Pi_stacking_P', 'Salt_Bridge', 'H_bond', 'Pi_Cation'])):
        if coord != HBond:
