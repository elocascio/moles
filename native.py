...
parser.add_argument("-absolute", action='store_true', help="absolute score using switching function")
...
if args.multi == False:
    ...
    if args.absolute == False:
        ...
        ax = sns.heatmap(times, yticklabels=list(zip(target_B.residues.resnums, target_B.residues.resnames)), cmap="Blues",vmin= 0, vmax = 1)
        ...
        for at in target_B.residues:
            ...
            #P.append([str(at.chainID) + str(at.residue.resnum) + str(at.residue.resname), np.mean(coord_P.timeseries[:, 1])])
            times.append(coord_P.timeseries[:,1])
        with open(f'./abs{args.sel_B}_with_{args.sel_A}.pkl', 'wb') as f: pkl.dump(times, f)
        plt.figure(figsize=(30,20))
        ax = sns.heatmap(times, yticklabels=list(zip(target_B.residues.resnums, target_B.residues.resnames)), cmap="Blues",vmin= 0)
        ax.set_title(f"{args.sel_A} vs {args.sel_B}", fontsize = 36)                
    ...
    if args.sub:
        ...
        dmatrix['mean'] = dmatrix.mean(axis=1); dmatrix['residue'] = list(zip(target_B.residues.resnums, target_B.residues.resnames))
        selected = dmatrix[dmatrix['mean'] < -0.3]
        print(selected[['residue', 'mean']])
        plt.figure(figsize=(30,20))
        plt.title(f"delta_{args.sel_A} vs {args.sel_B}")
        ax = sns.heatmap(dmatrix, yticklabels=list(zip(target_B.residues.resnums, target_B.residues.resnames)), cmap="RdBu", vmin= -1, vmax= 1)
            ...
            try:
                ...
                if args.absolute == False:
                    ...
                    for at in target_B.residues:
                        ...
                        #P.append([str(at.chainID) + str(at.residue.resnum) + str(at.residue.resname), np.mean(coord_P.timeseries[:, 1])])
                        times.append(coord_P.timeseries[:,1])
                    with open(f'{x}/abs{args.sel_B}_with_{args.sel_A}.pkl', 'wb') as f: pkl.dump(times, f)

                    plt.figure(figsize=(30,20))
                    ax = sns.heatmap(times, yticklabels=list(zip(target_B.residues.resnums, target_B.residues.resnames)), cmap="Blues",vmin= 0)
                    ax.set_title(f"{args.sel_A} vs {args.sel_B}", fontsize = 36)                
                    plt.savefig(f'{x}/abs_contacts_of_{args.sel_B}_with_{args.sel_A}.png', format = 'png')
                    plt.close()
                    print('matrix plot saved')
                ...
                if args.div and args.absolute:
                    ...
                    matrix = pd.DataFrame(times); matrix = matrix.fillna(0)  
                    rmatrix = pkl.load(open(args.ref, 'rb')); rmatrix = pd.DataFrame(rmatrix); rmatrix = rmatrix.fillna(0)
                    dmatrix = matrix.div(rmatrix); dmatrix = dmatrix.fillna(0)
                    plt.figure(figsize=(30,20))
                    ax = sns.heatmap(dmatrix, yticklabels=list(zip(target_B.residues.resnums, target_B.residues.resnames)), cmap="RdBu", vmin = 0, vmax = 1)
                    ax.set_title(f"{args.sel_B} vs {args.sel_A}", fontsize = 36)
                    plt.savefig(f"{x}/div_abs_matrix_{args.sel_B}_with_{args.sel_A}.png", format = 'png')
                    plt.close()
                    dmatrix['mean'] = dmatrix.mean(axis=1); dmatrix['residue'] = list(zip(target_B.residues.resnums, target_B.residues.resnames))
                    ...
                    if args.absolute: plt.savefig(f"{x}/abs_delta_matrix_{args.sel_B}_with_{args.sel_A}.png", format = 'png')
                    else: plt.savefig(f"{x}/delta_matrix_{args.sel_B}_with_{args.sel_A}.png", format = 'png')
                    plt.close()
                    dmatrix['mean'] = dmatrix.mean(axis=1); dmatrix['residue'] = list(zip(target_B.residues.resnums, target_B.residues.resnames))
