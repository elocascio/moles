...
if args.vsite:
    system(
        f"{args.gmx} pdb2gmx -ff charmm36m -f {args.system} -vsite {args.vsite} -o {args.system}_gmx.pdb -water tip3p -ignh -p topol.top"
    )
    print("IF YOU ARE USING VSITE, RISE THE STEP! 0.002 --> 0.004")
else:
    system(
        f"{args.gmx} pdb2gmx -ff charmm36m -f {args.system} -o {args.system}_gmx.pdb -water tip3p -ignh -p topol.top"
    )

system(
    f"{args.gmx} editconf -f {args.system}_gmx.pdb -o {args.system}_gmx.pdb -d {args.d} -quiet"
)
system(
    f"{args.gmx} solvate -cp {args.system}_gmx.pdb -o {args.system}_gmx.pdb -p topol.top -quiet"
)

ions_mdp = make_mdp("ions")
system(
    f"{args.gmx} grompp -f {ions_mdp} -c {args.system}_gmx.pdb -p topol.top -o Complex_b4ion.tpr -maxwarn 10 -quiet"
)
system(
