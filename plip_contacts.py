...
def contacts(pdb="MD.pdb", xtc="MD.xtc", step=10, ligand="UNK"):
    ...
    if ion != None:
        complexo = complesso + water + ion_group
        comp_ion = complesso + ion_group
    else:
        complexo = complesso + water
    ##################################### dictionaries #########################################################
    res_list = []
    if ion != None:
        for numres, nameres in list(zip(comp_ion.resnums, comp_ion.resnames)):
            res_list.append(str(numres) + str(nameres))
        dic = {key: [] for key in res_list}
    else:
        for numres, nameres in list(zip(complexo.resnums, complexo.resnames)):
            res_list.append(str(numres) + str(nameres))
        dic = {key: [] for key in res_list}
    Hydrophobic_dic, PiStacking_dic, PiCation_dic, HBond_dic, SaltBridge_dic = [dic] * 5
    ###########################################################################################################
    unk = u.select_atoms(f"resname {ligand}")
    lig_id = unk.resids[0]
    lig_chain = unk.chainIDs[0]
        ...
        for ts in u.trajectory:
            if ts.frame % step == 0:
                ...
                if ion != None:
                    ...
                    neg_atoms = un.select_atoms(
                        f"(resname {ligand}) and (name O* N* S* Cl* F* Br* I*)"
                    )
                    ion_group = un.select_atoms(f"resname {ion}")
                    metal_distance = np.linalg.norm(
                        neg_atoms.positions - ion_group.positions, axis=1
                    )
                    coordination_scores = (metal_distance < 3).astype(int)
                    for coordination_score in coordination_scores:
                        metal_coord.append(
                            [str(ion_group.resnames[0]), coordination_score]
                        )
                ####################################################
                ################# HYDROPHOBIC ########################
                df = pd.DataFrame()
                lig_c = u.select_atoms(f"resname {ligand} and name C*")
                prot_c = u.select_atoms(
                    f"protein and (around 7 resname {ligand}) and name C*"
                )
                ...
                for pi_stacking in my_interactions.pistacking:
                    ...
                    if pi_stacking.type == "T":
                        ...
                        PiStacking_T.append(
                            [str(pi_stacking.resnr) + str(pi_stacking.restype), t]
                        )
                    if pi_stacking.type == "P":
                        p = switch(float(pi_stacking.distance), r_0=5.5)
                        PiStacking_P.append(
                            [str(pi_stacking.resnr) + str(pi_stacking.restype), p]
                        )

                        stack = p + t
                        PiStacking_dic[
                            str(pi_stacking.resnr) + str(pi_stacking.restype)
                        ].append(stack)
                    for res in dic.keys():
                        if len(PiStacking_dic[res]) == counter:
                            pass
                        else:
                            PiStacking_dic[res].append(0)
                ##########################################################
                ################ SALT BRIDGE #############################
                for salt_bridge in my_interactions.saltbridge_lneg:
                    salt = switch(float(salt_bridge.distance), r_0=5)
                    SaltBridge.append(
                        [str(salt_bridge.resnr) + str(salt_bridge.restype), salt]
                    )
                    SaltBridge_dic[
                        str(salt_bridge.resnr) + str(salt_bridge.restype)
                    ].append(salt)
                ...
                for res in dic.keys():
                    if len(SaltBridge_dic[res]) == counter:
                        pass
                    else:
                        SaltBridge_dic[res].append(0)
                ######################################################
                ################ HBOND ###############################
                for h_bond in my_interactions.hbonds_ldon:
                    HBond.append(
                        [str(h_bond.resnr) + str(h_bond.restype), str(h_bond.type)]
                    )
                    if h_bond.type == "strong":
                        h = 1
                    elif h_bond.type == "weak":
                        h = 0.5
                ...
                for h_bond in my_interactions.hbonds_pdon:
                    ...
                    HBond_dic[str(h_bond.resnr) + str(h_bond.restype)].append(h)
                for res in dic.keys():
                    if len(HBond_dic[res]) == counter:
                        pass
                    else:
                        HBond_dic[res].append(0)
                #####################################################
                ################ PI CATION ##########################
                for picat in my_interactions.pication_laro:
                    PiCation.append(
                        [
                            str(picat.resnr) + str(picat.restype),
                            switch(float(picat.distance), r_0=4),
                        ]
                    )
                    PiCation_dic[str(picat.resnr) + str(picat.restype)].append(
                        switch(float(picat.distance))
                    )
                ...
                for res in dic.keys():
                    if len(PiCation_dic[res]) == counter:
                        pass
                    else:
                        PiCation_dic[res].append(0)
                ####################################################
                ################ WATER BRIDGE ######################
                for water_bridge in my_interactions.water_bridges:
                    WaterBridge.append(
                        [
                            str(water_bridge.resnr) + str(water_bridge.restype),
                            water_bridge.type,
                        ]
                    )
                ####################################################
    ...
    for coord, coord_type in list(
        ...
        if coord in [HBond, WaterBridge]:
            ...
            if args.abs:
                df[coord_type] = df[coord_type].apply(
                    lambda x: x / (u.trajectory.n_frames / step)
                )
                coord_dfs.append(df)
            else:
                df[coord_type] = df[coord_type].apply(
                    lambda x: x / df[coord_type].max()
                )
                coord_dfs.append(df)
    ...
    if args.abs:
        (ax.figure).savefig(
            f"coord_{ligand}_abs.png", format="png", bbox_inches="tight"
        )
    else:
